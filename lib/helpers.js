...
import { getReleasedBandNames, AudioPlayer, AKAs, getAlbumNames, getMusicianNames, getBandNames } from '../components/Muso';
...
const maybeReleaseLink = (artist='', title='') => {
	const bands = getReleasedBandNames();
	if (!bands.includes(artist)) return <><b>{artist}</b> - {title}</>;
	return (
