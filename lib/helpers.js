	...
	const getPlayer = ({ title, band, href, audio, mp3, author, comment, date, time }) => (
		<>
			...
			<audio controls="controls" title={title} preload="none" className="audio">
				<source src={`http://700west.com/audio/${audio || mp3}`} type="audio/mpeg" />
			</audio>
			*/}
			<a className="audio" href={`http://700west.com/audio/${audio || mp3}`}><img src='/images/lilspeaker.gif' width="11" height="11" alt="play audio" hspace="5" /></a>
			{getTitle(props)}
