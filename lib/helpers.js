...
const Item = ({ bold, author, info, date, body, extra, lyrics }, key) => (
...
const getBodyHTML = (body = '') => (body?.props?.children) ? body.props.children : body;

const Lyrics = (item) => {
	if (item.lyrics) {
		return (<pre className="row lyrics"><div className="title">Lyrics</div>{item.lyrics}</pre>)
	}
}

export { Lyrics, getBodyHTML, Topper, SectionHeader, makeMusicianLink, makeReleaseLink, AudioPlayer, typeToDisplay, makeSubject, FormatDate, makeDate, Item, makeAuthor, dateCompare, makeBandLink, makeSource };
